...
/
ALTER TABLE preregistroContacto ADD CONSTRAINT preregistroContacto_pk PRIMARY KEY (numpreregistro,CONTACTO);
/
...
IF inserting THEN
  --IF :NEW."NUMPREREGISTRO" IS NULL THEN
    SELECT PREREGISTROContacto_P_SEQ.nextval INTO :NEW."NUMPREREGISTRO" FROM dual;
  --END IF;
END IF;
END;
/
...
INSERT ON "PREREGISTROCONTACTO" FOR EACH row BEGIN
IF inserting THEN
 -- IF :NEW."CONTACTO" IS NULL THEN
    SELECT PREREGISTROCONTACTO_C_SEQ.nextval INTO :NEW."CONTACTO" FROM dual;
 -- END IF;
END IF;
END;
/
